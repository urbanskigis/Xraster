...
Install
Operating system:   Windows 10 (version 2009), 11    (at this time only)
...
The installation process of Xraster in QGIS follows these steps:
    ...
    • Open QGIS 3.xx (minimum version 3.20) - QGIS should be  install using OSGeo4W installer with selected packages QGIS and GDAL (at least)
...
Since the plugin requires additional Python libraries that are not installed by default, if they are not found, a message will be displayed, ModuleNotFoundError
...
In such a case, you should:
  ...
  You need to install the packages: xarray, netCDF4, h5netcdf, scipy (NEW in version 4) and Cartopy using the following four commands executed in sequence:
...
Data import - Input of NetCDF data consisting of 2D raster grids with a time dimension. 
Subset - Creates a subset of a selected variable for a specified range of dimensions: spatial coordinates, time, and depth. 
Calculate - Recalculates the current subset variable using a Python expression. 
Filtering - Filter of the data subset using comparison and logical operators. 
Time Summary - Calculates temporal statistics from surface data (x, y, time) and from point data (z, time).
Spatial Summary - Calculates statistics from all available data (subset, mask, filtering) at a specified time.
...
Manual and Tutorial
The manual  Xraster_QGIS_Plugin_user_manual_EN_03.pdf  and the tutorial  Xraster_tutorial_v3.pdf together with the accompanied data, are aveilable.  
Contribute
Jacek Urbański
Licence
