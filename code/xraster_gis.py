...
class XrasterPlug:
    ...
    def __init__(self, iface):
        ...
        self.dlg.flattbot.clicked.connect(partial(self.runTOOL,"flatt")) # Flatten variable to bottom         
        ...
        self.dlg.RegionCR.clicked.connect(partial(self.runTOOL,"Reg"))
        self.dlg.RegPlot1B.clicked.connect(partial(self.runTOOL,"RegPlot1"))
        self.dlg.RegPlotID.clicked.connect(partial(self.runTOOL,"RegPlot2"))
        self.dlg.RegPlotdaysID.clicked.connect(partial(self.runTOOL,"RegPlot3"))
        self.dlg.RegPlotAT.clicked.connect(partial(self.runTOOL,"RegPlot4"))
        self.dlg.regSavePlot.clicked.connect(partial(self.runTOOL,"RegSS"))
        self.dlg.regExp.clicked.connect(partial(self.runTOOL,"RegEx"))
        ...
        self.dlg.regfilesB.clicked.connect(self.select_output_file21)
        self.dlg.regSavefileB.clicked.connect(self.select_output_file22) 
        self.dlg.reggeofileB.clicked.connect(self.select_output_file23)
        self.dlg.regartsB.clicked.connect(self.select_output_file24) 
    ...
    def select_output_file21(self):
        """ Output region results files .csv _sum.csv file"""
        global ppath
        filenamet2=QFileDialog.getSaveFileName(self.dlg,"Export data file .csv",ppath,'*.csv')
        filename2=str(filenamet2[0])
        if filename2!='':
            ppath=os.path.dirname(filename2)
        self.dlg.regfiles.setText(filename2)   


    def select_output_file22(self):
        global ppath
        filenamet2=QFileDialog.getSaveFileName(self.dlg,"Export plot file",ppath,'*.jpg')
        filename2=str(filenamet2[0])
        if filename2!='':
            ppath=os.path.dirname(filename2)
        self.dlg.regSavefile.setText(filename2) 
        self.dlg.regSavePlot.setEnabled(True)

    def select_output_file23(self):
        """ Open mask .tif file"""
        global ppath
        filenamet2=QFileDialog.getSaveFileName(self.dlg,"Export map to geotif",ppath,'*.tif')
        filename2=str(filenamet2[0])
        if filename2!='':
            ppath=os.path.dirname(filename2)
        self.dlg.reggeofile.setText(filename2)
        self.dlg.regExp.setEnabled(True)
        
    def select_output_file24(self):
        """ Output region results files .csv _sum.csv file"""
        global ppath
        filenamet2=QFileDialog.getSaveFileName(self.dlg,"Export data file .csv",ppath,'*.csv')
        filename2=str(filenamet2[0])
        if filename2!='':
            ppath=os.path.dirname(filename2)
        self.dlg.regarts.setText(filename2)  
        self.dlg.regExp.setEnabled(True)        
#-------------------------------------------        
#-----------------------------------------------------------------------------        
    def runTOOL(self,mflow):
        ...
        global ds22C
        global ds222
        global ds222ST,ds222g,ds222gsum
        ...
        global current_label, globlab
        global sout_sum,sout
        global gtype
        ...
        def AcBu(mflow):
            if mflow=='data':
                ...
                self.dlg.regfiles.setText(sscl)
            ...
            if mflow=='data_ord':
                ...
                if COORN==4:
                    self.dlg.flattbot.setEnabled(True)
        ...
        def summarize_data(file_path: str, output_path: str):
            """
            Reads a text file with columns (date,id,number_of_cells),
            computes summary per id, and writes result to a new file.
            
            Output columns: id, max_number_of_cells, date_start, date_end, date_of_max
            Sorted by max_number_of_cells (descending).
            """
            # Load file
            df = pd.read_csv(file_path, header=None, names=["date", "id", "number_of_cells"])
            ...
            summary = (
                df.groupby("id").apply(
                    lambda g: pd.Series({
                        "max_number_of_cells": g["number_of_cells"].max(),
                        "date_start": g["date"].min(),
                        "date_end": g["date"].max(),
                        "date_of_max": g.loc[g["number_of_cells"].idxmax(), "date"]
                    })
                )
                .reset_index()
                .sort_values(by="max_number_of_cells", ascending=False)
            )
        ...
        def label_connected_components(array):
        ...
        def label_connected_components_8(array):
            global current_label
            unique_values = np.unique(array)
            output = np.zeros_like(array, dtype=int)
            #current_label = 1
            ...
            # Define an 8-connected neighborhood structure
            ...
            for value in unique_values:
                ...
                output += labeled_mask
                current_label += num_features
                #print('xxx',current_label)

            return output
            
        

        def merge_groups2(arr1: np.ndarray, arr2: np.ndarray) -> Tuple[np.ndarray, Dict[int, int]]:
            """
            Merge groups from arr2 into arr1 based on overlap rules.
            Ignores NaN and 0 as background.

            Parameters:
                arr1 (np.ndarray): array with initial groups (ids like 1,2,3…).
                arr2 (np.ndarray): array with new groups (ids like 4,5…).
            
            Returns:
                arr3 (np.ndarray): merged array with final group ids.
                mapping (Dict[int, int]): dictionary mapping arr2 ids to final ids.
            """
            from scipy.ndimage import label
            ...
            def get_groups(arr):
                ...
                valid = valid[valid != 0]  # exclude 0
                for val in np.unique(valid):
                    mask = (arr == val)
                    labeled, n = label(mask)
                    for g in range(1, n+1):
                        coords = tuple(zip(*np.where(labeled == g)))
                        groups[(val, g)] = set(coords)
                return groups

            groups1 = get_groups(arr1)
            groups2 = get_groups(arr2)

            arr3 = np.full_like(arr2, np.nan, dtype=float)  # start with NaN
            mapping = {}

            for (val2, g2), coords2 in groups2.items():
                overlap_counts = {}
                for (val1, g1), coords1 in groups1.items():
                    overlap = coords2 & coords1
                    if overlap:
                        overlap_counts[(val1, g1)] = len(overlap)
                if overlap_counts:
                    best_group = max(overlap_counts, key=overlap_counts.get)
                    chosen_id = best_group[0]
                else:
                    chosen_id = val2

                mapping[val2] = chosen_id
                for (i, j) in coords2:
                    arr3[i, j] = chosen_id

            return arr3

        def extract_sorted_integers(da: xr.DataArray) -> list[int]:
            """
            Extract sorted list of integer values from an xarray.DataArray,
            excluding zeros and NaNs.
            """
            # Convert to numpy array
            values = da.values.flatten()
            
            # Remove NaN, filter out zeros, cast to int
        ...
        def count_cells_with_value(da: xr.DataArray, ii: int, value: int) -> int:
            """
            Count number of cells equal to a specific value in ds222[ii].

            Parameters
            ----------
            da : xr.DataArray
                The analysed_sst DataArray (e.g., ds222).
            ii : int
                The index along the first dimension (e.g., time).
            value : int
                The value to count.
            ...
            -------
            int
                Number of cells matching the given value.
            """
            # Select the slice
            slice_da = da.isel({da.dims[0]: ii})
            
            # Convert to numpy
            arr = slice_da.values
            
            # Count matches (ignoring NaN automatically by numpy)
            count = np.sum(arr == value)
            
            return int(count)

    
        def get_date_range_by_id(file_path, target_id):
            """
            Reads a text file with columns:
            id,max_number_of_cells,date_start,date_end,date_of_max
            and returns (date_start, date_end) for the given id.
            """
            with open(file_path, "r") as file:
                # Skip the header line if present
                header = file.readline().strip()
                # Check if header contains text, not data
                ...
                for line in file:
                    ...
                    record_id, _, date_start, date_end, _ = parts
                    if str(record_id).strip() == str(target_id):
                        return date_start, date_end

            return None  # Return None if ID not found

        def load_data_for_id(file_path: str, target_id: int): # -> pd.DataFrame:
            """
            Reads the text file and returns a DataFrame with
            'date' and 'number_of_cells' for the given id.
            
            Parameters:
                file_path (str): Path to the input text/CSV file.
                target_id (int): The ID to filter by.
            
            Returns:
                pd.DataFrame: DataFrame with columns ['date', 'number_of_cells'].
            """
            # Read the file (assuming comma-separated)
            df = pd.read_csv(file_path, header=None, names=["date", "id", "number_of_cells"])
            
            # Filter by target id
            filtered_df = df[df["id"] == target_id][["date", "number_of_cells"]]
            
            return filtered_df

        def flatten_to_max_depth(ds, var_name):
            """
            Flatten a 4D variable (depth, lat, lon, time) to the deepest valid (non-NaN) depth.

            Automatically detects the depth dimension name.
            Keeps all original variables in ds.
            Skips creating 'max_depth' or '<var_name>_max_depth' if they already exist.

            Parameters
            ----------
            ds : xarray.Dataset
                Dataset containing the variable and a vertical (depth-like) dimension.
            var_name : str
                Name of the variable to flatten (e.g., 'o2', 'temperature').

            Returns
            -------
            ds_out : xarray.Dataset
                Dataset including all original variables plus:
                  - {var_name}_max_depth: value at deepest valid depth (if newly created)
                  - max_depth: corresponding depth (if not already present)
            """

            # --- Step 1: Detect depth dimension name ---
            depth_dim = None
            for candidate in ['depth', 'Depth', 'z', 'Z', 'lev', 'level']:
                if candidate in ds.dims:
                    depth_dim = candidate
                    break
            if depth_dim is None:
                raise ValueError("No depth-like dimension found (expected one of: depth, Depth, z, Z, lev, level).")

            depth = ds[depth_dim]
            var = ds[var_name]
            out_name = f"{var_name}_max_depth"

            # --- Step 2: Check if already exists ---
            if out_name in ds and 'max_depth' in ds:
                print(f"⏭️  Skipping: both '{out_name}' and 'max_depth' already exist.")
                return ds
            elif out_name in ds:
                print(f"⏭️  Skipping: '{out_name}' already exists.")
                return ds

            # --- Step 3: Compute valid mask and deepest index ---
            valid_mask = var.notnull()
            valid_reversed = valid_mask.isel({depth_dim: slice(None, None, -1)})
            max_depth_idx = valid_reversed.argmax(dim=depth_dim)
            max_depth_idx = len(depth) - 1 - max_depth_idx

            # Identify all-NaN profiles
            all_nan = valid_mask.sum(dim=depth_dim) == 0
            max_depth_idx = max_depth_idx.where(~all_nan)

            # --- Step 4: Convert to integer for indexing ---
            max_depth_idx_int = max_depth_idx.fillna(0).astype(int)

            # --- Step 5: Helper for selecting value at index ---
            def pick_value(arr, idx):
                if np.isnan(idx):
                    return np.nan
                return arr[int(idx)]

            # --- Step 6: Extract variable at deepest depth ---
            var_max_depth = xr.apply_ufunc(
                pick_value,
                var,
                max_depth_idx,
            ...
            # --- Step 7: Compute max_depth only if missing ---
            if 'max_depth' not in ds:
                max_depth_values = xr.apply_ufunc(
                    pick_value,
                    depth.broadcast_like(var),
                    max_depth_idx,
                    input_core_dims=[[depth_dim], []],
                ...
                max_depth_values = ds['max_depth']

            # --- Step 8: Create new dataset (preserving all variables) ---
            ds_out = ds.copy()
            ds_out[out_name] = var_max_depth
            if 'max_depth' not in ds:
                ds_out['max_depth'] = max_depth_values

            # --- Step 9: Preserve attributes ---
            ds_out[out_name].attrs = var.attrs
            if 'max_depth' not in ds:
                ds_out['max_depth'].attrs = depth.attrs

            print(f"✅ Added '{out_name}'" + (" and 'max_depth'" if 'max_depth' not in ds else ""))
            ...
            try:
                ...
                if COORN==3:
                    ...
                    for i in range(len(var_list)):
                        self.dlg.varBoxA.addItem(var_list[i])
                #self.dlg.order.setText(sss)
                order_sss=sss
                if 'mask' in ds2.variables:
                    ds2 = ds2.rename({'mask': 'mask_1'})
            except:
        ...
        if mflow=='select':   
            ...
            self.dlg.RegionCR.setEnabled(False); regflag=0
            if COORN==4:
                szet=str(zmin_val)+","+str(zmax_val)
           ...
           # self.dlg.labelSEL.setText(sss)
            ...
            if COORN==3:
                ...
                srest=''
                if COORN==3 and stype==1:
                    srest='Slices of space and time'
                elif COORN==3 and stype==2:
                    srest='Slice of space at a single time'
                elif COORN==3 and stype==3:
                    srest='Point in space and time slice'
                elif COORN==3 and stype==4:
                    srest='Point in space at a single time'   
                elif COORN==4 and stype==1:
                    srest='Slices of space,time and depth '
                elif COORN==4 and stype==11:
                    srest='Slices of space and time for a single depth'
                elif COORN==4 and stype==2:
                    srest='Slice of space at a single time  and depth'
                elif COORN==4 and stype==22:
                    srest='Slices of space and depth at a single time'
                elif COORN==4 and stype==3:
                    srest='Point in space, single depth, and time slice'
                elif COORN==4 and stype==33:
                    srest='Point in space with slices of time and depth'
                elif COORN==4 and stype==4:
                    srest='Point in space at a single time and depth'
                elif COORN==4 and stype==44:
                    srest='Point in space with a slice of depth at a single time'                    
                    ...
                    if xdd==0:
                        ...
                        if calc_erflag:
                           sss='Calculation error'
                            ...
                            self.dlg.labelSEL.setText(sss)
                            self.dlg.labelSType.setText(srest)
                        ...
                        if stype==1:
                           ...
                           max_val = ds222.max().item()
                           if max_val==1:    # only for var 0,1            
                                regflag=1 
                                self.dlg.RegionCR.setEnabled(True) 
                           else:
                                regflag=0
                                self.dlg.RegionCR.setEnabled(False) 
                        elif stype==2:
            ...
            self.dlg.akm2lab.setText('')
            self.dlg.akm2lab.setText('')
            # calculate anomaly
            
                        
            if self.dlg.anomalyB.isChecked() and ((stype==1 and COORN==3) or (stype==11 and COORN==4)):
                #ds222=ds222-ds222.mean()
                sds222=f"ds222-ds222.{'mean'}(dim='{l_c[l_ci[2]]}')"      
                code = compile(sds222, "<string>", "eval")
                ds222=eval(code,{'ds222':ds222}) 
            
            # calculate area in km2
            if 'km2' in ds2.data_vars:
                ds222A=ds222*0+ds2.km2
                akm2=ds222A.sum().max().item()
                sakm2=str(round(akm2,2))
                self.dlg.akm2lab.setText(sakm2)
                ds222A=None
            # calculate number of cells
            ds222A=ds222*0+1
            cnr=ds222A.sum().max().item()
            scnr=str(round(cnr,0))
            self.dlg.cnrlab.setText(scnr)
            ds222A=None                
        ...
        if mflow=='Reg':
            # region groups run
            gtype=0
            sout=self.dlg.regfiles.text()          
            max_val = ds222.max().item()
            if max_val==1 and sout!='':    # only for var 0,1
                self.dlg.progressBar_reg.setValue(0)
                ii=0
                iik=ds222["time"].size
                current_label=1
                globlab=current_label

                # from first not all NaN
                arr = ds222.isel(time=ii)
                while arr.isnull().all():
                    ii=ii+1
                    arr = ds222.isel(time=ii)
                procc=0;licznik=0
                krok=(iik-ii)/100    

                for i in range(ii,iik-1):
                    arr = ds222.isel(time=i)
                    if not arr.isnull().all():
                        if i==ii:
                            licznik=licznik+1
                            AA1=ds222[i].to_numpy()
                            AA1G = label_connected_components_8(AA1)
                            AA1G[AA1==0]=0
                            AA1G=np.ma.masked_where(np.isnan(AA1), AA1G) 
                            ds222[dict(time=i)] = AA1G
                            if np.all(np.isnan(AA1G)):
                                max_val = None  # or np.nan
                            else:
                                max_val = np.nanmax(AA1G)
                                current_label=int(max_val+1)
                                if globlab>current_label:
                                    current_label=globlab+1
                                globlab=current_label 
                            #print(i,current_label)
                        else:
                            licznik=licznik+1
                            if licznik>=krok:
                                procc+=1
                                self.dlg.progressBar_reg.setValue(procc)
                                licznik=0 
                            
                            AA1=ds222[i-1].to_numpy()
                            AA2=ds222[i].to_numpy()
                            #print('para',i-1,i)
                            
                            AA2G = label_connected_components_8(AA2)
                            
                            AA3G  =  merge_groups2(AA1,AA2G)
                            AA3G[AA2==0]=0
                            AA3G=np.ma.masked_where(np.isnan(AA2), AA3G) 
                            ds222[dict(time=i)] = AA3G
                            if np.all(np.isnan(AA3G)):
                                max_val = None  # or np.nan
                            else:
                                max_val = np.nanmax(AA3G)
                                current_label=int(max_val+1)
                                if globlab>current_label:
                                    current_label=globlab+1
                                globlab=current_label 
                
                regf=self.dlg.regfiles.text()
                # create regions description file
                out_ff=open(sout,"w")
                for iras in range(ii,iik):
                    sorted_values = extract_sorted_integers(ds222[iras])
                    time_at_tt = ds222['time'].values[iras]
                    rtime=str(time_at_tt)
                    for i in range(len(sorted_values)):
                        num_cells = count_cells_with_value(ds222, ii=iras, value=sorted_values[i])
                        sss=f'{rtime[:10]},{sorted_values[i]},{num_cells}\n'
                        out_ff.write(sss)
                out_ff.close()
                #  create regions description file  
                sout=Path(sout)
                sout_sum = sout.with_stem(sout.stem + "_sum")
                summary_df = summarize_data(sout, sout_sum)
                procc=100
                self.dlg.progressBar_reg.setValue(procc) 
                self.dlg.RegPlot1B.setEnabled(True)
                self.dlg.RegPlotID.setEnabled(True)
                self.dlg.RegPlotdaysID.setEnabled(True)
                self.dlg.RegPlotAT.setEnabled(True)
        ...
        if mflow=='RegPlot1':
            ...
            if len(stime)!=0:    
                ...
                if self.dlg.Npolarst.isChecked() or self.dlg.Spolarst.isChecked():
                    ...
                    ds222.sel(time=stime).plot(size=figsize,aspect=fasp)
                    plt.tight_layout()
                plt.show()
        ...
        if mflow=='RegPlot2':
            gtype=2
            sIDD=self.dlg.sID_reg.text()
            stime=self.dlg.sTIME_reg.text()
            if (len(stime)!=0) & (len(sIDD)!=0):
                gdate=stime
                gid=int(sIDD)
                ds222gm=ds222.sel(time=gdate)

                moper=f'*0'
                smoper='ds222gm'+moper
                code = compile(smoper, "<string>", "eval")
                ds222gg=eval(code,{'ds222gm':ds222gm})
                ds222gm=ds222gg
                ds222g=ds222.sel(time=gdate)
                moper=f'*0+ds222g=={gid}'
                smoper='ds222g'+moper
                code = compile(smoper, "<string>", "eval")
                ds222gg=eval(code,{'ds222g':ds222g})
                ds222g=ds222gg+ds222gm
                ...
                if self.dlg.Npolarst.isChecked() or self.dlg.Spolarst.isChecked():
                    ...
                    ds222g.plot(size=figsize,aspect=fasp)
                    plt.tight_layout()
                plt.show()
        ...
        if mflow=='RegPlot3':
            ...
            sIDD=self.dlg.sID_reg.text()
            gid=int(sIDD)
            gresult = get_date_range_by_id(sout_sum,gid)
            if gresult is not None:
                gdate1=gresult[0];gdate2=gresult[1]
            
                ds222gm=ds222.sel(time=gdate1)
                ...
                ds222g=ds222.sel(time=slice(gdate1,gdate2))
                ...
                ds222gsum=ds222g.sum(dim='time')+ds222gm
                ...
                if self.dlg.Npolarst.isChecked() or self.dlg.Spolarst.isChecked():
                    ...
                    ds222gsum.plot(size=figsize,aspect=fasp);
                    plt.title('time '+gdate1+' - '+gdate2)
                    plt.tight_layout()
                plt.show()                
        ...
        if mflow=='RegPlot4':
                    ...
                    if gresult is not None:
                        gdate1=gresult[0];gdate2=gresult[1]
                        df = load_data_for_id(sout, gid)
                        cellkm2=1;flagkm2=0
                        if 'km2' in ds2.data_vars:
                            cellkm2=ds2['km2'].mean(skipna=True).item()
                            flagkm2=1
                        df['date'] = pd.to_datetime(df['date'])
                        df = df.set_index("date")
                        df['number_of_cells'] *= cellkm2
                        xr_data=df.to_xarray()
                        if flagkm2==0:
                            xr_data["number_of_cells"].plot(size=figsize,aspect=fasp);
                        else:    
                            xr_data["number_of_cells"].rename("area in km²").plot(size=figsize,aspect=fasp);
                        plt.show()
        ...
        if mflow=='RegSS':
            ...
            fdpi=float(self.dlg.figDpi.text())
            plt.rcParams.update({'font.size': ffont})  
            figname=self.dlg.regSavefile.text() 
            plt.savefig(figname,dpi=fdpi);


        if mflow=="RegEx":
            fn2=self.dlg.reggeofile.text()
            if (gtype>0 and gtype<4) and mmask==1:
                # ds222ST   - array 2D; fn - path to mask; fn2 -output raster
                yy,xx=extract_floats(slat,slon) 
                if gtype==1: 
                    stime=self.dlg.sTIME_reg.text()
                    nnp=ds222.sel(time=stime).to_numpy()   
                if gtype==2: nnp=ds222g.to_numpy()  
                if gtype==3: nnp=ds222gsum.to_numpy() 
                xy_extent=nnp.shape
                gdal.AllRegister() 
                ds=gdal.Open(fn,gdal.GA_ReadOnly)
                if ds is None:
            ...
            if gtype==4: 
                    ffregarts=self.dlg.regarts.text()
                    sIDD=self.dlg.sID_reg.text()
                    gid=int(sIDD)
                    gresult = get_date_range_by_id(sout_sum,gid)
                    if gresult is not None:
                        ...
                        cellkm2=1;flagkm2=0
                        if 'km2' in ds2.data_vars:
                            cellkm2=round(ds2['km2'].mean(skipna=True).item(),3)
                            flagkm2=1
                        df['date'] = pd.to_datetime(df['date'])
                        df = df.set_index("date")
                        df['number_of_cells'] *= cellkm2 
                        if 'km2' in ds2.data_vars:
                            df.rename(columns={'number_of_cells': 'area_in_km2'}, inplace=True)
                        df.to_csv(ffregarts, index=True)
                    
        if mflow=="flatt":
                    varr=self.dlg.varBoxA.currentText()
                    ds2=flatten_to_max_depth(ds2, varr)
                    self.runTOOL("data_ord")
        ...
        if mflow=="manual":
            pdf_url = "https://github.com/urbanskigis/Xraster/blob/main/Xraster_QGIS_Plugin_user_manual_EN_04.pdf"  # Use an absolute path
